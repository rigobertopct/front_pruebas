      ...
      <div class="card shadow-lg">
        ...
        <div class="table-responsive">
          <table id="datatable-basic" class="table table-flush">
            ...
            <tbody>
            <tr v-for="deporte in deportesPorNombre" :key="deporte.id">
              ...
              <td class="text-sm font-weight-normal ps-4">
                <div class="btn-group" role="group">
                  ...
                  <button @click="openUpdate(deporte.id, deporte.nombre)"
...
const BUSCAR = gql`query DeportesPorNombre($name:String!) {
    deportesPorNombre(name: $name) {
        id
        nombre
        isActivo
    }
}`;
const NUEVO = gql`mutation NuevoDeporte($nombre:String!) {
    nuevoDeporte(nombre: $nombre) {
        success
        error
    }
}`;
const ACTUALIZAR = gql`mutation ActualizarDeporte($id:Int!, $nombre:String!) {
    actualizarDeporte(id: $id, nombre: $nombre) {
        success
        error
    }
}`;
  ...
  methods: {
    ...
    async Actualizar() {
      ...
      await this.$apollo.mutate({
        ...
        if (response.data.actualizarDeporte.success) {
          const Toast = Swal.mixin({
            toast: true,
            position: "top-end",
            showConfirmButton: false,
          ...
          Toast.fire({
            icon: "error",
            title: response.data.actualizarDeporte.error
          });
        }
        this.closeUpdate();
      }).catch(error => {

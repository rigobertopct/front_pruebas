  ...
  <div class="mt-4 row">
      ...
      <div class="card shadow-lg">
        ...
        <div class="table-responsive">
          <table id="datatable-basic" class="table table-flush">
            ...
            <tbody>
            <tr v-for="item in areaPorNombre" :key="item.id">
              <td class="text-sm font-weight-normal ps-4">{{ item.nombre }}</td>
              <td class="text-sm font-weight-normal ps-4">{{ item.sigla }}</td>
              <td class="text-sm font-weight-normal ps-4">{{ item.departamento.nombre }}</td>
              <td class="text-sm font-weight-normal ps-4">{{ item.responsable.firstName }}
              ...
              <td class="text-sm font-weight-normal ps-4">
                <div class="btn-group" role="group">
                  ...
                  <button
                    @click="openUpdate(item.id, item.nombre, item.sigla, item.departamento.id, item.responsable.id )"
      ...
      <div v-if="showModal" :class="['modal', { 'show': showModal }]" @transitionend="onTransitionEnd">
        <div class="modal-content">
          <div class="row mb-3 border-bottom border-dark">
            <h4 class="text-start"><i class="fa fa-plus-square me-2"></i>Nueva Área<i @click="closeModal"
          ...
          <div class="row mb-3">
            <div class="col-12">
              ...
              <select v-model="departamento" class="form-control border-bottom border-dark p-2 mb-3">
                <option value="0" disabled>Seleccione un Departamento</option>
                <option v-for="elemento in allDepartamento" :key="elemento.id" :value="elemento.id">{{ elemento.nombre
                  }}
                </option>
              </select>
              <label class="form-label">Responsable</label>
              <select v-model="responsable" class="form-control border-bottom border-dark p-2">
                <option value="0" disabled>Seleccione un Responsable</option>
                <option v-for="elemento in allUsers" :key="elemento.id" :value="elemento.id">
    ...
    <div class="divModal Actualizar">
      <div v-if="showUpdate" :class="['modal', { 'show': showUpdate }]" @transitionend="onTransitionEnd">
        <div class="modal-content">
          <div class="row mb-3 border-bottom border-dark">
            <h4 class="text-start"><i class="fa fa-pencil-square me-2"></i>Actualizar Área<i
              @click="closeUpdate"
              class="material-icons-round opacity-10 modal-icon">close</i>
            </h4>
          </div>
...
const USERS = gql`query AllUsers {
    allUsers {
        id
        lastLogin
        username
...
`;
const DEPARTAMENTOS = gql`query AllDepartamento {
    allDepartamento {
        id
        nombre
        sigla
...
const BUSCAR = gql`query AreaPorNombre($name:String!) {
    areaPorNombre(name: $name) {
        id
        nombre
        sigla
        isActivo
...
`;
const NUEVO = gql`mutation NuevaArea($departamento:Int!, $responsable:Int!,$nombre:String!, $siglas:String!) {
    nuevaArea(departamento: $departamento, nombre: $nombre, responsable: $responsable, siglas: $siglas) {
        success
        error
    }
}`;
const ACTUALIZAR = gql`mutation ActualizarArea($id:Int!, $departamento:Int!, $responsable:Int!,$nombre:String!, $siglas:String!) {
    actualizarArea(id:$id, departamento: $departamento, nombre: $nombre, responsable: $responsable, siglas: $siglas) {
        success
        error
    }
}`;
export default {
  ...
  methods: {
    async Guardar() {
      if (this.nombre === "" || this.siglas === "" || this.departamento === 0 || this.responsable === 0) {
        this.error = true;
        return false;
      }
      await this.$apollo.mutate({
        ...
        if (response.data.nuevaArea.success) {
    ...
    async Actualizar() {
      ...
      await this.$apollo.mutate({
        ...
        if (response.data.actualizarArea.success) {
          const Toast = Swal.mixin({
            toast: true,
            position: "top-end",
            showConfirmButton: false,
          ...
          Toast.fire({
            icon: "error",
            title: response.data.actualizarArea.errors
          });
        }
        this.closeUpdate();
      }).catch(error => {
    ...
    openUpdate(id, nombre, siglas, departamento, responsable) {
      this.id = id;
      this.nombre = nombre;
      this.siglas = siglas;

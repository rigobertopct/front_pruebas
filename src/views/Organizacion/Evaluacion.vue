  ...
  <div class="mt-4 row">
      ...
      <div class="card shadow-lg">
        ...
        <div class="table-responsive">
          <table id="datatable-basic" class="table table-flush">
            ...
            <tbody>
            <tr v-for="item in evaluacionPorNombre" :key="item.id">
              ...
              <td class="text-sm font-weight-normal ps-4">
                <div class="btn-group" role="group">
                  ...
                  <button @click="openUpdate(item.id, item.nombre)"
      ...
      <div v-if="showModal" :class="['modal', { 'show': showModal }]" @transitionend="onTransitionEnd">
        <div class="modal-content">
          <div class="row mb-3 border-bottom border-dark">
            <h4 class="text-start"><i class="fa fa-plus-square me-2"></i>Nueva evaluación<i @click="closeModal"
    ...
    <div class="divModal Actualizar">
      <div v-if="showUpdate" :class="['modal', { 'show': showUpdate }]" @transitionend="onTransitionEnd">
        <div class="modal-content">
          <div class="row mb-3 border-bottom border-dark">
            <h4 class="text-start"><i class="fa fa-pencil-square me-2"></i>Actualizar evaluación<i
              @click="closeUpdate"
              class="material-icons-round opacity-10 modal-icon">close</i>
            </h4>
          </div>
...
const BUSCAR = gql`query EvaluacionPorNombre($name:String!) {
    evaluacionPorNombre(name: $name) {
         id
        nombre
        isActivo
    }
}`;
const NUEVO = gql`mutation NuevaEvaluacion($nombre:String!) {
    nuevaEvaluacion(nombre: $nombre) {
        success
        error
    }
}`;
const ACTUALIZAR = gql`mutation ActualizarEvaluacion($id:Int!, $nombre:String!) {
    actualizarEvaluacion(id: $id, nombre: $nombre) {
        success
        error
    }
}`;
  ...
  methods: {
    async Guardar() {
      ...
      await this.$apollo.mutate({
        ...
        if (response.data.nuevaEvaluacion.success) {
          ...
          Toast.fire({
            icon: "error",
            title: response.data.nuevaEvaluacion.error
          });
        }
        this.closeModal();
      }).catch(error => {
    ...
    async Actualizar() {
      ...
      await this.$apollo.mutate({
        ...
        if (response.data.actualizarEvaluacion.success) {
          const Toast = Swal.mixin({
            toast: true,
            position: "top-end",
            showConfirmButton: false,
          ...
          Toast.fire({
            icon: "error",
            title: response.data.actualizarEvaluacion.error
          });
        }
        this.closeUpdate();
      }).catch(error => {

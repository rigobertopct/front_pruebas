  ...
  <div class="mt-4 row">
      ...
      <div class="card shadow-lg">
        ...
        <div class="table-responsive">
          <table id="datatable-basic" class="table table-flush">
            ...
            <tbody>
            <tr v-for="item in lugarPorNombre" :key="item.id">
      ...
      <div v-if="showModal" :class="['modal', { 'show': showModal }]" @transitionend="onTransitionEnd">
        <div class="modal-content">
          <div class="row mb-3 border-bottom border-dark">
            <h4 class="text-start"><i class="fa fa-plus-square me-2"></i>Nuevo lugar<i @click="closeModal"
    ...
    <div class="divModal Actualizar">
      <div v-if="showUpdate" :class="['modal', { 'show': showUpdate }]" @transitionend="onTransitionEnd">
        <div class="modal-content">
          <div class="row mb-3 border-bottom border-dark">
            <h4 class="text-start"><i class="fa fa-pencil-square me-2"></i>Actualizar lugar<i
              @click="closeUpdate"
              class="material-icons-round opacity-10 modal-icon">close</i>
            </h4>
          </div>
...
const BUSCAR = gql`query LugarPorNombre($name:String!) {
    lugarPorNombre(name: $name) {
         id
        nombre
        isActivo
    }
}`;
const NUEVO = gql`mutation NuevoLugar($nombre:String!) {
    nuevoLugar(nombre: $nombre) {
        success
        error
    }
}`;
const ACTUALIZAR = gql`mutation ActualizarLugar($id:Int!, $nombre:String!) {
    actualizarLugar(id: $id, nombre: $nombre) {
        success
        error
    }
}`;
  ...
  methods: {
    ...
    async Actualizar() {
      ...
      await this.$apollo.mutate({
        ...
        if (response.data.actualizarLugar.success) {
          const Toast = Swal.mixin({
            toast: true,
            position: "top-end",
            showConfirmButton: false,
          ...
          Toast.fire({
            icon: "error",
            title: response.data.actualizarLugar.error
          });
        }
        this.closeUpdate();
      }).catch(error => {

  ...
  <div class="mt-4 row">
      ...
      <div class="card shadow-lg">
        ...
        <div class="table-responsive">
          <table id="datatable-basic" class="table table-flush">
            ...
            <tbody>
            <tr v-for="item in servicioPorNombre" :key="item.id">
              <td class="text-sm font-weight-normal ps-4">{{ item.nombre }}</td>
              <td class="text-sm font-weight-normal ps-4">{{ item.simbolo }}</td>
              <td class="text-sm font-weight-normal ps-4">{{ item.objetivo }}</td>
              <td class="text-sm font-weight-normal ps-4">{{ item.area.nombre }}</td>
              <td class="text-sm font-weight-normal ps-4">
                <div class="btn-group" role="group">
                  ...
                  <button @click="openUpdate(item.id, item.nombre, item.objetivo, item.simbolo, item.area.id)"
        ...
        <div class="modal-content">
          ...
          <div class="row mb-3">
            <div class="col-12">
              ...
              <input v-model="simbolo" class="form-control border-bottom border-dark p-2"
                     type="text">
              <label class="form-label">Objetivo</label>
              <input v-model="objetivo" class="form-control border-bottom border-dark p-2"
                     type="text">
              <label class="form-label">Área</label>
              <select v-model="area" class="form-control">
                <option value="0" disabled>Seleccione un área</option>
                <option v-for="item in allArea" :key="item.id" :value="item.id">{{ item.nombre }}</option>
              </select>
    ...
    <div class="divModal Actualizar">
      <div v-if="showUpdate" :class="['modal', { 'show': showUpdate }]" @transitionend="onTransitionEnd">
        <div class="modal-content">
          <div class="row mb-3 border-bottom border-dark">
            <h4 class="text-start"><i class="fa fa-pencil-square me-2"></i>Actualizar servicio<i @click="closeUpdate"
                                                                                                 class="material-icons-round opacity-10 modal-icon">close</i>
            </h4>
          </div>
          <div class="row mb-3">
...
const BUSCAR = gql`query ServicioPorNombre($name:String!) {
    servicioPorNombre(name: $name) {
        id
        nombre
        simbolo
        objetivo
...
const NUEVO = gql`mutation NuevoServicio($area: Int!, $nombre: String!, $objetivo: String!, $simbolo: String!) {
    nuevoServicio(area: $area, nombre: $nombre, objetivo: $objetivo, simbolo: $simbolo) {
        success
        errors
    }
}`;
const ACTUALIZAR = gql`mutation AtualizarServicio($id:Int!, $area: Int!, $nombre: String!, $objetivo: String!, $simbolo: String!) {
    actualizarServicio(area: $area, nombre: $nombre, objetivo: $objetivo, simbolo: $simbolo, id:$id) {
        success
        errors
    }
}`;
const ALLAREAS = gql`query AllArea {
    allArea {
        id
        nombre
        sigla
...
export default {
  ...
  methods: {
    async Guardar() {
      if (this.nombre === "" || this.simbolo === "" || this.simbolo === "" || this.area === 0) {
        this.error = true;
        return false;
      }
      await this.$apollo.mutate({
        ...
        if (response.data.nuevoServicio.success) {
          ...
          Toast.fire({
            icon: "error",
            title: response.data.nuevoServicio.error
          });
        }
        this.closeModal();
      }).catch(error => {
    ...
    async Actualizar() {
      ...
      await this.$apollo.mutate({
        ...
        if (response.data.actualizarServicio.success) {
          const Toast = Swal.mixin({
            toast: true,
            position: "top-end",
            showConfirmButton: false,
          ...
          Toast.fire({
            icon: "error",
            title: response.data.actualizarServicio.error
          });
        }
        this.closeUpdate();
      }).catch(error => {
    ...
    openUpdate(id, nombre, objetivo, simbolo, area) {
      this.id = id;
      this.nombre = nombre;

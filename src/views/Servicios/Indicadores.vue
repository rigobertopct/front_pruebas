  ...
  <div class="mt-4 row">
        ...
        <div class="table-responsive">
          <table id="datatable-basic" class="table table-flush">
            ...
            <tr v-for="item in indicadores" :key="item.id">
              <td class="text-sm font-weight-normal ps-4">{{ item.indicador }}</td>
              <td class="text-sm font-weight-normal ps-4">{{ item.unidad }}</td>
              <td class="text-sm font-weight-normal ps-4">{{ item.servicio.nombre }}</td>
              <td class="text-sm font-weight-normal ps-4">
                <div class="btn-group" role="group">
                  ...
                  <button @click="openUpdate(item.id, item.indicador, item.unidad, item.servicio.id)"
      ...
      <div v-if="showModal" :class="['modal', { 'show': showModal }]" @transitionend="onTransitionEnd">
        <div class="modal-content">
          <div class="row mb-3 border-bottom border-dark">
            <h4 class="text-start"><i class="fa fa-plus-square me-2"></i>Nuevo indicador<i @click="closeModal"
          ...
          <div class="row mb-3">
            <div class="col-12">
              ...
              <input v-model="indicador" class="form-control border-bottom border-dark p-2"
                     type="text">
              <label class="form-label">Unidad de medida</label>
              <input v-model="unidad" class="form-control border-bottom border-dark p-2"
                     type="text">
              <label class="form-label">Servicio</label>
              <select v-model="servicio" class="form-control">
                <option value="0" disabled>Seleccione un servicio</option>
                <option v-for="item in allServicio" :key="item.id" :value="item.id">{{ item.nombre }}</option>
              </select>
    ...
    <div class="divModal Actualizar">
      <div v-if="showUpdate" :class="['modal', { 'show': showUpdate }]" @transitionend="onTransitionEnd">
        <div class="modal-content">
          <div class="row mb-3 border-bottom border-dark">
            <h4 class="text-start"><i class="fa fa-pencil-square me-2"></i>Actualizar indicador<i @click="closeUpdate"
                                                                                                class="material-icons-round opacity-10 modal-icon">close</i>
            </h4>
          </div>
          <div class="row mb-3">
...
const BUSCAR = gql`query Indicadores($name: String!) {
    indicadores(name: $name) {
        id
        indicador
        unidad
        servicio {
...
const NUEVO = gql`mutation NuevoIndicador($indicador: String!, $servicio: Int!, $unidad: String!) {
    nuevoIndicador(indicador: $indicador, servicio: $servicio, unidad: $unidad) {
        success
        errors
    }
}
`;
const ACTUALIZAR = gql`mutation ActualizarIndicador($id:Int!, $indicador: String!, $servicio: Int!, $unidad: String!) {
    actualizarIndicador(id:$id, indicador: $indicador, servicio: $servicio, unidad: $unidad) {
        success
        errors
    }
}`;
const ALLSERVICIOS = gql`query AllServicio {
    allServicio {
        id
        nombre
        simbolo
...
export default {
  ...
  methods: {
    async Guardar() {
      if (this.indicador === "" || this.unidad === "" || this.servicio === 0) {
        this.error = true;
        return false;
      }
      await this.$apollo.mutate({
        ...
        if (response.data.nuevoIndicador.success) {
          ...
          Toast.fire({
            icon: "error",
            title: response.data.nuevoIndicador.error
          });
        }
        this.closeModal();
      }).catch(error => {
    ...
    async Actualizar() {
      ...
      await this.$apollo.mutate({
        ...
        if (response.data.actualizarIndicador.success) {
          const Toast = Swal.mixin({
            toast: true,
            position: "top-end",
            showConfirmButton: false,
          ...
          Toast.fire({
            icon: "error",
            title: response.data.actualizarIndicador.error
          });
        }
        this.closeUpdate();
      }).catch(error => {
    ...
    openUpdate(id, indicador, unidad, servicio) {
      this.id = id;
